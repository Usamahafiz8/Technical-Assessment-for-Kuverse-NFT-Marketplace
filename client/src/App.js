...
function App() {
  ...
  useEffect(() => {
    ...
    if (connectorKey) {
      const savedConnector = getConnector(connectorKey);
      activate(savedConnector, (error) => {
        const errorMsg = getErrorMessage(error);
        const errType = getErrorType(error);
        setNetworkError(errorMsg);
        setErrorType(errType);
        setErrorModalOpen(true);
        window.localStorage.removeItem(connectorLocalStorageKey); // Clear invalid connector
      });
    }
  }, [activate]);
  ...
  const connectToProvider = (connector) => {
    ...
    activate(connector);
    setConnectModalOpen(false); // Close modal after connecting
  };
        ...
        <>
          ...
          <MainConetent>
            <Navbar
              ...
              account={account} // Pass account to Navbar for display
            />
            ...
            <ModalContainer>
              <ModalWrap className="modal-wrap">
                <WModalBody>
                  ...
                  >
                    {account ? shorter(account) : "Not connected"} {/* Display wallet address */}
                  </h5>
                  <div>
                    <div style={{ display: "flex", flexDirection: "column" }}>
